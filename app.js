...
const copy = {
  de: {
    edit:"Struktur bearbeiten", eyebrow:"Interaktiver Leitfaden · Stand 2026",
    ...
    heroPrimary:"Unsere Zielstruktur", heroSecondary:"Grundlagen verstehen",
    basicsKicker:"01 · Grundlagen", basicsTitle:"Was ist eine GitHub Organization?",
    ...
    rolesKicker:"02 · Ownership & Rechte", rolesTitle:"Zwei Ebenen, die man nicht verwechseln sollte",
    orgRolesTitle:"Organization-Rollen", repoRolesTitle:"Repository-Rollen",
    orgRoles:[
    ...
    repoRoles:[
      ["Read","Lesen und diskutieren."],["Triage","Issues und Pull Requests verwalten, ohne Code zu schreiben."],
      ["Write","Aktiv Code pushen und entwickeln."],["Maintain","Repository verwalten, ohne alle sensiblen Admin-Rechte."],
    ...
    enterpriseKicker:"03 · Große Unternehmen", enterpriseTitle:"Wie skalierende Unternehmen GitHub normalerweise aufbauen",
    ...
    enterpriseSteps:[
      ["Enterprise / Firma","zentrale Governance, Richtlinien, Abrechnung"],["Organizations","z. B. Produkte, Bereiche oder Sicherheitszonen"],
      ["Teams","Development, Marketing, External, Security"],["Repositories","Produkte, Websites, Services, Infrastruktur"],
      ["Projects & CI/CD","Aufgaben, Automationen, Deployments"]
    ],
    targetKicker:"04 · Unser Modell", targetTitle:"Die Struktur für Agentur, TFV und Fures",
    ...
    owner:"Owner", members:"Mitglieder / technische Mitarbeit", repos:"Beispiel-Repositories", responsibility:"Geschäftliche Verantwortung",
    ...
    repoKicker:"05 · Repo-Strategie", repoTitle:"Monorepo oder mehrere Repositories?",
    ...
    deployKicker:"06 · Netlify & Deployments", deployTitle:"Ein Repository bedeutet nicht automatisch ein gemeinsames Deployment",
    ...
    freeKicker:"07 · Kosten", freeTitle:"Für euren Start reicht GitHub Free sehr weit",
    ...
    migrationKicker:"08 · Umsetzung", migrationTitle:"So würde ich die Umstellung durchführen",
    migration:[
    ...
    faqKicker:"09 · FAQ", faqTitle:"Die typischen Fragen",
    faq:[
    ...
    sourcesKicker:"10 · Quellen", sourcesTitle:"Offizielle GitHub-Dokumentation", footer:"Interner Lern- und Architekturleitfaden",
    editorKicker:"Live-Konfiguration", editorTitle:"Organisationen und Ownership bearbeiten",
    ...
    reset:"Standard wiederherstellen", export:"JSON exportieren", import:"JSON importieren", save:"Speichern & anwenden", saved:"Gespeichert.", invalid:"JSON ist ungültig. Bitte Syntax prüfen."
  },
  tr: {
    edit:"Yapıyı düzenle", eyebrow:"Etkileşimli rehber · 2026",
    ...
    heroPrimary:"Bizim hedef yapımız", heroSecondary:"Temeli öğren",
    basicsKicker:"01 · Temel", basicsTitle:"GitHub Organization nedir?",
    ...
    rolesKicker:"02 · Sahiplik & Yetki", rolesTitle:"Birbirine karıştırılmaması gereken iki seviye",
    orgRolesTitle:"Organization rolleri", repoRolesTitle:"Repository rolleri",
    orgRoles:[
    ...
    repoRoles:[
      ["Read","Okuma ve tartışmalara katılma."],["Triage","Kod yazmadan Issue ve Pull Request yönetimi."],
      ["Write","Aktif geliştirme ve push yetkisi."],["Maintain","Hassas Admin yetkileri olmadan repository yönetimi."],
    ...
    enterpriseKicker:"03 · Büyük şirketler", enterpriseTitle:"Büyüyen şirketler GitHub'ı genelde nasıl kuruyor?",
    ...
    enterpriseSteps:[
      ["Enterprise / Şirket","merkezi politika, güvenlik ve faturalama"],["Organizations","ürünler, iş alanları veya güvenlik bölgeleri"],
      ["Teams","Development, Marketing, External, Security"],["Repositories","ürün, web sitesi, servis ve altyapılar"],
      ["Projects & CI/CD","iş takibi, otomasyon ve deploy"]
    ],
    targetKicker:"04 · Bizim modelimiz", targetTitle:"Agentur, TFV ve Fures için hedef yapı",
    ...
    owner:"Owner", members:"Üyeler / teknik çalışma", repos:"Örnek repository'ler", responsibility:"İş tarafındaki sorumluluk",
    ...
    repoKicker:"05 · Repo stratejisi", repoTitle:"Monorepo mu, ayrı repository'ler mi?",
    ...
    deployKicker:"06 · Netlify & Deploy", deployTitle:"Tek repository, tek deploy demek değildir",
    ...
    freeKicker:"07 · Maliyet", freeTitle:"Başlangıç için GitHub Free oldukça yeterli",
    ...
    migrationKicker:"08 · Uygulama", migrationTitle:"Geçişi böyle yapardım",
    migration:[
    ...
    faqKicker:"09 · SSS", faqTitle:"En çok karıştırılan konular",
    faq:[
    ...
    sourcesKicker:"10 · Kaynaklar", sourcesTitle:"Resmi GitHub dokümantasyonu", footer:"Dahili eğitim ve mimari rehber",
    editorKicker:"Canlı konfigürasyon", editorTitle:"Organization ve sahiplikleri düzenle",
    ...
    reset:"Varsayılana dön", export:"JSON dışa aktar", import:"JSON içe aktar", save:"Kaydet & uygula", saved:"Kaydedildi.", invalid:"JSON geçersiz. Sözdizimini kontrol et."
  }
};

function clone(v){ return JSON.parse(JSON.stringify(v)); }
function loadStructure(){
  try{return JSON.parse(localStorage.getItem(STORAGE_KEY)) || clone(window.GITHUB_GUIDE_DEFAULTS);}
  catch{return clone(window.GITHUB_GUIDE_DEFAULTS);}
}
...
function render(){
  ...
  setText("edit-structure",c.edit); setText("hero-eyebrow",c.eyebrow); setText("hero-title",c.heroTitle); setText("hero-copy",c.heroCopy);
  setText("hero-primary",c.heroPrimary); setText("hero-secondary",c.heroSecondary);

  structure.people.slice(0,3).forEach((p,i)=>setText("diagram-person-"+["a","b","c"][i],p));
  $("hero-org-row").innerHTML=structure.organizations.map(o=>`<div class="org-chip ${esc(o.tone)}"><strong>${esc(o.name)}</strong><small>${esc(c.responsibility)}: ${esc(o.responsibility)}</small></div>`).join("");

  setText("basics-kicker",c.basicsKicker); setText("basics-title",c.basicsTitle); setText("basics-intro",c.basicsIntro);
  $("basics-cards").innerHTML=c.basics.map(x=>`<article class="card"><div class="icon">${x[0]}</div><h3>${x[1]}</h3><p>${x[2]}</p></article>`).join("");

  setText("roles-kicker",c.rolesKicker); setText("roles-title",c.rolesTitle); setText("org-roles-title",c.orgRolesTitle); setText("repo-roles-title",c.repoRolesTitle);
  ...
  setText("enterprise-kicker",c.enterpriseKicker); setText("enterprise-title",c.enterpriseTitle); setText("enterprise-copy",c.enterpriseCopy);
  $("enterprise-flow").innerHTML=c.enterpriseSteps.map(x=>`<div class="enterprise-step"><strong>${x[0]}</strong><small>${x[1]}</small></div>`).join("");

  setText("target-kicker",c.targetKicker); setText("target-title",c.targetTitle); setText("target-copy",c.targetCopy);
  $("target-grid").innerHTML=structure.organizations.map(o=>`<article class="target-org ${esc(o.tone)}"><div class="org-title"><h3>${esc(o.name)}</h3><span class="pill">${esc(c.responsibility)}: ${esc(o.responsibility)}</span></div><dl><dt>${esc(c.owner)}</dt><dd>${(o.owners||[]).map(esc).join(", ") || "—"}</dd><dt>${esc(c.members)}</dt><dd>${(o.members||[]).map(esc).join(", ") || "—"}</dd></dl><strong>${esc(c.repos)}</strong><div class="repo-list">${(o.repositories||[]).map(r=>`<span>▰ ${esc(r)}</span>`).join("")}</div></article>`).join("");
  setText("target-legend",c.targetLegend);

  setText("repo-kicker",c.repoKicker); setText("repo-title",c.repoTitle); setText("repo-copy",c.repoCopy);
  $("repo-compare").innerHTML=c.compare.map((x,i)=>`<article class="compare-card ${i===0?"recommended":""}"><h3>${x[0]}</h3><p>${x[1]}</p><ul class="compare-list">${x[2].map(v=>`<li>${v}</li>`).join("")}</ul></article>`).join("");

  setText("deploy-kicker",c.deployKicker); setText("deploy-title",c.deployTitle); setText("deploy-copy",c.deployCopy); setText("deploy-notice",c.deployNotice);

  setText("free-kicker",c.freeKicker); setText("free-title",c.freeTitle);
  $("free-stats").innerHTML=c.stats.map(x=>`<div class="stat"><strong>${x[0]}</strong><span>${x[1]}</span></div>`).join("");
  setText("free-note",c.freeNote);

  setText("migration-kicker",c.migrationKicker); setText("migration-title",c.migrationTitle);
  $("migration-list").innerHTML=c.migration.map(x=>`<li><strong>${x[0]}</strong><span>${x[1]}</span></li>`).join("");

  setText("faq-kicker",c.faqKicker); setText("faq-title",c.faqTitle);
  $("faq-list").innerHTML=c.faq.map(x=>`<details><summary>${x[0]}</summary><p>${x[1]}</p></details>`).join("");

  setText("sources-kicker",c.sourcesKicker); setText("sources-title",c.sourcesTitle); setText("footer-copy",c.footer);
  setText("editor-kicker",c.editorKicker); setText("editor-title",c.editorTitle); setText("editor-help",c.editorHelp);
  setText("reset-structure",c.reset); setText("export-structure",c.export); setText("import-label",c.import); setText("save-structure",c.save);
}
...
$("edit-structure").addEventListener("click",()=>{$("structure-json").value=JSON.stringify(structure,null,2);$("editor-status").textContent="";$("editor-dialog").showModal();});
$("save-structure").addEventListener("click",()=>{try{const next=JSON.parse($("structure-json").value);if(!Array.isArray(next.organizations)||!Array.isArray(next.people)) throw new Error("schema");structure=next;localStorage.setItem(STORAGE_KEY,JSON.stringify(structure));$("editor-status").textContent=copy[lang].saved;render();}catch{$("editor-status").textContent=copy[lang].invalid;}});
$("reset-structure").addEventListener("click",()=>{structure=clone(window.GITHUB_GUIDE_DEFAULTS);localStorage.removeItem(STORAGE_KEY);$("structure-json").value=JSON.stringify(structure,null,2);render();});
$("export-structure").addEventListener("click",()=>{const blob=new Blob([JSON.stringify(structure,null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="github-organization-structure.json";a.click();URL.revokeObjectURL(url);});
$("import-structure").addEventListener("change",async(e)=>{const file=e.target.files?.[0];if(!file)return;try{const parsed=JSON.parse(await file.text());$("structure-json").value=JSON.stringify(parsed,null,2);$("editor-status").textContent="";}catch{$("editor-status").textContent=copy[lang].invalid;}});
render();